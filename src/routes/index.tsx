      ...
      <div className="jt">
        ...
        <main className="page">
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"0"}}>
            ...
            {(()=>{
              const renderCourse=(courseId:'basic-share-market'|'option-trading',label:string,desc:string,accentColor:string,purchased:boolean)=>{
                const c=COURSES[courseId];
                const expanded=expandedCourse===courseId;
                const topics=curriculum[courseId]||[];
                if(!courseList.some(x=>x.id===courseId)) return null;
                return(
                  <div key={courseId} style={{gridColumn:'1/-1',marginBottom:"20px"}}>
                    <article className={`courseNew ${courseId==='basic-share-market'?'basicCourse':'optionCourse'}`} style={{borderRadius:expanded?"18px 18px 0 0":"18px"}}>
                      <div className="courseVisual" style={c.thumbnailKey?{backgroundImage:`url(/api/media?key=${encodeURIComponent(c.thumbnailKey)})`,backgroundSize:'cover',backgroundPosition:'center'}:{}}>
                        <span>{c.status==='coming_soon'?'COMING SOON':label}</span>
                      </div>
                      <div className="courseBody">
                        <h3 style={{fontSize:"clamp(18px,2.5vw,26px)"}}>{courseId==='basic-share-market'?'Basic of Share Market':'Option Trading Course'}</h3>
                        <p>{desc}</p>
                        <div className="courseBottom" style={{flexWrap:"wrap",gap:"10px"}}>
                          {c.status==='coming_soon'
                            ? <b style={{color:'#f5c842',fontSize:"16px"}}>Coming Soon</b>
                            : purchased
                              ? <button onClick={()=>{setDashCourseTab(courseId);setTab("dashboard");}}>Start Learning →</button>
                    ...
                    {expanded&&(
                      <div style={{background:"rgba(8,15,35,.9)",border:`1px solid ${accentColor}33`,borderTop:"none",borderRadius:"0 0 18px 18px"}}>
                        <div style={{padding:"14px 22px 8px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <b style={{fontSize:"11px",color:"#f0f4fc",letterSpacing:"1px"}}>COURSE CURRICULUM</b>
                          <span style={{fontSize:"10px",color:"#5f7a9a"}}>{topics.length} topics</span>
                        </div>
                        {topics.length===0
                          ? <p style={{color:"#5f7a9a",fontSize:"12px",textAlign:"center",padding:"24px"}}>Loading…</p>
                          : topics.map((l,i)=>(
                            <div key={l.id} style={{display:"flex",gap:"14px",alignItems:"center",padding:"13px 22px",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
                              <span style={{color:accentColor,fontSize:"11px",fontWeight:700,minWidth:"24px",flexShrink:0}}>{String(i+1).padStart(2,'0')}</span>
                              <div style={{flex:1,minWidth:0}}>
                                <b style={{fontSize:"13px",color:"#f0f4fc",display:"block",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{l.title}</b>
                        ...
                        {!purchased&&c.status!=='coming_soon'&&(
                          <div style={{padding:"18px 22px"}}>
                            <button onClick={()=>{setViewCourse(courseId);setTab("course");}} style={{width:"100%",background:`linear-gradient(135deg,${accentColor},${accentColor}cc)`,color:'#fff',padding:"14px",borderRadius:"10px",fontSize:"13px",fontWeight:900,border:"none",cursor:"pointer"}}>
                              Enroll in this course — {c.priceStr}
                            </button>
                          </div>
                        )}
                      </div>
              ...
              return(
                <>
                  {renderCourse('basic-share-market','BEGINNER FRIENDLY','Understand how the share market works, key concepts, how to read stock prices and analysis basics.','#3b82f6',basicPurchased)}
                  {renderCourse('option-trading','MOST POPULAR','Learn options, strategies, risk management and practical trade execution with real market examples.','#d97706',optionPurchased)}
                </>
              );
            })()}
    ...
    <div className="jt">
      ...
      <section className="heroNew">
        ...
        <div className="founderStage">
          ...
          <div className="founderFrame" style={{background:"linear-gradient(160deg,#0a1628 0%,#0d2045 50%,#0a1628 100%)",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",position:"relative"}}>
            <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 60% 40%,rgba(212,175,55,.12),transparent 65%)"}}/>
            <div style={{position:"relative",textAlign:"center",padding:"30px"}}>
              <div style={{fontSize:"clamp(56px,8vw,90px)",fontWeight:950,letterSpacing:"-4px",lineHeight:.85,background:"linear-gradient(135deg,#fff 30%,#d4af37 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>J</div>
              <div style={{fontSize:"11px",letterSpacing:"6px",color:"#d4af37",marginTop:"14px",fontWeight:700}}>JTRADER ACADEMY</div>
              <div style={{width:"40px",height:"2px",background:"linear-gradient(90deg,transparent,#d4af37,transparent)",margin:"12px auto"}}/>
              <div style={{fontSize:"10px",letterSpacing:"2px",color:"rgba(212,175,55,0.5)",fontStyle:"italic"}}>Learn · Earn · Grow</div>
            </div>
          </div>
      ...
      <section style={{width:"min(1160px,92%)",margin:"0 auto 80px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"60px",alignItems:"start"}}>
        ...
        <div>
          ...
          <p style={{color:"#94a3b8",lineHeight:1.85,fontSize:"15px",marginBottom:"28px"}}>
          ...
          <div style={{borderLeft:"3px solid #d4af37",paddingLeft:"20px"}}>
            <p style={{margin:0,fontSize:"18px",fontWeight:800,color:"#f0f4fc",lineHeight:1.7}}>
              Learn and earn.<br/>
...
const glassCss=`
/* ── Slide-in transition ── */
@keyframes jtIn{from{opacity:0;transform:translateX(48px)}to{opacity:1;transform:translateX(0)}}
.jt{animation:jtIn .42s cubic-bezier(.16,1,.3,1)}

/* ── Ambient 3-D background ── */
body{background:#020408}
.jt{background:radial-gradient(ellipse 70% 55% at 8% -5%,rgba(18,50,110,.45) 0%,transparent 100%),radial-gradient(ellipse 55% 45% at 92% 105%,rgba(10,55,25,.35) 0%,transparent 100%),#020408}

/* ── Glassmorphism nav ── */
nav{background:rgba(3,6,12,.88)!important;backdrop-filter:blur(32px) saturate(1.6)!important;border-bottom:1px solid rgba(255,255,255,.055)!important;box-shadow:0 4px 30px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.035)!important}
.bar{background:rgba(3,5,10,.92)!important;backdrop-filter:blur(20px)!important}

/* ── Glassmorphism panels ── */
.lessonList,.enroll,.signup,.community,.locked,.table,.stats>div,.card,.feature,.step,.courseGrid aside,.manageCard,.settingsCard{background:rgba(7,13,22,.62)!important;backdrop-filter:blur(22px)!important;border:1px solid rgba(255,255,255,.065)!important;box-shadow:0 8px 36px rgba(0,0,0,.52),inset 0 1px 0 rgba(255,255,255,.04)!important}
.lessonList header{background:transparent!important;backdrop-filter:none!important}

/* ── Offer banner glass ── */
.offerCycleBanner{background:rgba(9,18,38,.72)!important;backdrop-filter:blur(22px)!important;border:1px solid rgba(80,130,255,.18)!important;box-shadow:0 8px 36px rgba(0,0,0,.45),0 0 70px rgba(50,90,200,.06)!important}

/* ── Buttons — lift on hover ── */
.primary,.heroPrimary{background:linear-gradient(135deg,#fff 0%,#dce9f8 100%)!important;color:#080a0d!important;box-shadow:0 4px 18px rgba(255,255,255,.14),0 2px 6px rgba(0,0,0,.35)!important;transition:transform .2s cubic-bezier(.16,1,.3,1),box-shadow .2s!important}
.primary:hover,.heroPrimary:hover{transform:translateY(-3px) scale(1.02)!important;box-shadow:0 10px 32px rgba(255,255,255,.22),0 4px 10px rgba(0,0,0,.4)!important}
.primary:active,.heroPrimary:active{transform:translateY(-1px)!important;box-shadow:0 4px 14px rgba(255,255,255,.12)!important}
.navBtn{transition:transform .18s,box-shadow .18s!important}
.navBtn:hover{transform:translateY(-2px)!important;box-shadow:0 6px 18px rgba(0,0,0,.4)!important}
.outline:hover{transform:translateY(-2px)!important;border-color:rgba(255,255,255,.25)!important}

/* ── Course cards — 3D lift ── */
.courseNew{transition:transform .3s cubic-bezier(.16,1,.3,1),box-shadow .3s!important}
.courseNew:hover{transform:translateY(-8px) scale(1.015)!important;box-shadow:0 30px 80px rgba(0,0,0,.65)!important}
.courseNew .courseBottom button{transition:transform .2s,box-shadow .2s!important}
.courseNew .courseBottom button:hover{transform:translateY(-2px)!important;box-shadow:0 6px 18px rgba(0,0,0,.4)!important}

/* ── Lesson rows — slide right on hover ── */
.lessonClickable{transition:transform .18s cubic-bezier(.16,1,.3,1),background .18s!important}
.lessonClickable:hover{transform:translateX(5px)!important;background:rgba(14,24,42,.85)!important}

/* ── Stats cards glow on hover ── */
.stats>div{transition:border-color .25s,box-shadow .25s!important}
.stats>div:hover{border-color:rgba(80,130,255,.22)!important;box-shadow:0 0 40px rgba(60,100,220,.1),0 8px 36px rgba(0,0,0,.55)!important}

/* ── Gradient headings ── */
.welcomeDashboard h1,.pageHero h1{background:linear-gradient(120deg,#f0f6ff 30%,#7aa8d0 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.hero h1{background:linear-gradient(110deg,#fff 20%,#a8c4e0 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}

/* ── Input glow on focus ── */
input:focus,textarea:focus,select:focus{border-color:rgba(90,140,255,.45)!important;box-shadow:0 0 0 3px rgba(65,110,220,.1)!important;outline:none!important}

/* ── Video modal depth ── */
.videoModal{backdrop-filter:blur(6px)!important}
.videoModalInner{background:rgba(8,13,22,.92)!important;backdrop-filter:blur(30px)!important;box-shadow:0 40px 100px rgba(0,0,0,.75),0 0 0 1px rgba(255,255,255,.06)!important}

/* ── Footer glass ── */
footer{background:rgba(3,5,10,.85)!important;backdrop-filter:blur(20px)!important;border-top:1px solid rgba(255,255,255,.04)!important}
